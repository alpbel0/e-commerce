...
public interface ReviewResponseRepository extends JpaRepository<ReviewResponse, UUID> {
    ...
    List<ReviewResponse> findByReviewIdAndActiveTrueOrderByCreatedAtAsc(UUID reviewId);

    boolean existsByReviewIdAndActiveTrue(UUID reviewId);

    List<ReviewResponse> findByReviewIdAndActiveTrue(UUID reviewId);
}
